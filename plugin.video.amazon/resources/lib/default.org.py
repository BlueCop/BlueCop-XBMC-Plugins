...
def ROOT():
    ...
    updatetv.append( ('Fix Genres',   "XBMC.RunPlugin(%s?mode=fixGenres)" % ( sys.argv[0] ) ) )
    updatetv.append( ('Fix Years',   "XBMC.RunPlugin(%s?mode=fixYears)" % ( sys.argv[0] ) ) )
...
def LIST_TVSHOWS(HDonly=False,genrefilter=False,creatorfilter=False,networkfilter=False,yearfilter=False,favorfilter=False):
    ...
    for seriestitle,plot,creator,network,genres,actors,year,stars,votes,episodetotal,watched,unwatched,isHD,isprime,favor,TVDBbanner,TVDBposter,TVDBfanart in shows:
        ...
        if editenable == 'true':
            cm.append( ('Rename Show', "XBMC.RunPlugin(%s?mode=renameShowdb&title=%s)" % ( sys.argv[0], urllib.quote_plus(seriestitle) ) ) )
            cm.append( ('Remove Show', "XBMC.RunPlugin(%s?mode=deleteShowdb&title=%s)" % ( sys.argv[0], urllib.quote_plus(seriestitle) ) ) )
...
def LIST_TV_SEASONS(HDonly=False):
    ...
    for url,poster,season,seriestitle,plot,creator,network,genres,actors,year,stars,votes,episodetotal,watched,unwatched,isHD,isprime in seasons:
        ...
        cm = []
        if editenable == 'true':
            cm.append( ('Rename Season', "XBMC.RunPlugin(%s?mode=renameSeasondb&title=%s&season=%s)" % ( sys.argv[0], urllib.quote_plus(seriestitle), str(season) ) ) )
